...
exports.uploadReport = async (req, res) => {
  try {
    ...
    const dateStr = today.toISOString().split("T")[0]; // e.g. 2025-10-06
...
//     const terminalMapByCode = {};
//     const terminalMapByName = {};

//     rows.forEach(row => {
//       if (row.terminal_code) terminalMapByCode[row.terminal_code.trim()] = row;
//       if (row.merchant_name) terminalMapByName[row.merchant_name.trim().toLowerCase()] = row;
//     });

//     const exchangeRate = 137; // Birr → USD

//     // 4. Merge Excel with DB data (match by terminal ID, else by merchant name)
//     const mergedData = excelData.map(item => {
//       const terminalId = item["TERMINAL ID"] ? item["TERMINAL ID"].trim() : null;
//       const merchantName = item["MER_ENSE"] ? item["MER_ENSE"].trim().toLowerCase() : null;

//       // Try to find matching DB record
//       let dbInfo = terminalMapByCode[terminalId] || terminalMapByName[merchantName] || {};
...
//       return {
//         merchant_name: item["MER_ENSE"] || "Unknown",
//         terminal_id: item["TERMINAL ID"] || "N/A",
//         sum_local_txn: item["SUM LOCAL TXN"] || 0,
//         sum_local_txn_amount: item["SUM LOCAL TXN AMNT"] || 0,
//         sum_visa_txn: item["SUM VISA TXN"] || 0,
//         sum_visa_amount: visaAmount,
